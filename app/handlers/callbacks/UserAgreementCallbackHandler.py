...
class UserAgreementCallbackHandler(AbstractCallbackHandler):
    ...
    async def process(callback_query: types.CallbackQuery, *args, **kwargs):
        ...
        if UserHelper.is_user_agreement_accepted(
            user_telegram_id=user_telegram_id
        ):
            return await app.bot.answer_callback_query(
                callback_query.id,
                text='Пользовательское соглашение уже принято.',
                show_alert=True,
            )
        ...
        answer_message = await app.bot.send_message(
            user_telegram_id, 'Пользовательское соглашение принято!'
        )
        await StartMenu.show(
            chat_id=answer_message.chat.id, telegram_user_id=user_telegram_id
        )
